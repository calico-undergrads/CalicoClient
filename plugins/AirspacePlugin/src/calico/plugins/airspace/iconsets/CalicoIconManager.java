package calico.plugins.airspace.iconsets;
...
public class CalicoIconManager {
	...
	private static String iconThemeName = ""; // Set in constructor

	public static int defaultIconSize = 16;

	public static Logger logger = Logger.getLogger(CalicoIconManager.class
			.getName());

	public static void setIconTheme(Class<?> clazz, String name) {
		iconThemeName = name;
		try {
			iconTheme.load(clazz.getResourceAsStream("/calico/iconsets/"
					+ iconThemeName + "/icontheme.properties"));
		...
		logger.info("Loading Icon Theme: "
				+ iconTheme.getProperty("iconset.name") + " by "
				+ iconTheme.getProperty("author.name") + " ("
	...
	public static String getIcon(String icon) {
		...
		URL url = CalicoDataStore.calicoObj.getClass().getResource(
				"iconsets/" + iconThemeName + "/" + iconPath);
		return url.toString();
	}
	...
	public static Image getIconImage(String iconPath) {
		try {
			...
			Image icon = toolkit.getImage(new URL(CalicoIconManager
					.getIcon(iconPath)));
	...
	public static BufferedImage getImagePart(Image img, int x, int y, int w,
			int h) {
		BufferedImage bgBuf = new BufferedImage(img.getWidth(null),
